...
public partial class ItemStorage
{
	...
	public bool CanInsertItemPartially(object? user, int slot, Item? item, out int leftOver)
	{
		...
		{
			if (item.type != storageItem.type || !ItemLoader.CanStack(storageItem, item))
			{
				return false;
			}
	...
	/// </summary>
	/// <returns>
	/// True if the item was successfully inserted. False if the item is air, if the slot is already
